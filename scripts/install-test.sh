...
if [ -n "$LOCAL_BINARY_DIR" ]; then
    if [ -z "$requested_version" ]; then
        echo -e "${RED}VERSION must be set when using LOCAL_BINARY_DIR${NC}"
        exit 1
    fi
    specific_version=$requested_version
    echo -e "${MUTED}Using local binary from: ${NC}$LOCAL_BINARY_DIR"
...
download_and_install() {
    ...
    if [ -n "$LOCAL_BINARY_DIR" ]; then
        echo -e "${MUTED}Copying local binary...${NC}"
        local local_file="$LOCAL_BINARY_DIR/$filename"
        if [ ! -f "$local_file" ]; then
            echo -e "${RED}Local binary not found: $local_file${NC}"
            exit 1
        fi
